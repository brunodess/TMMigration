...
Script para remocao de OSCE usando CUT e instalacao de Apex SaaS. Este verifica arquitetura do SO e o hostname para escolher o instalador
correto a ser executado. O instalador é baicado diretamente da console Web e executado no endpoint apos execucao do CUT. Este tambem
tenta remover Vulnerability Protection Agent e Endpoint Sensor Standalone, caso existam.
...
function startScript {
    ...
    $global:LogFileLocal = ("c:\temp\_Apex_$dataLog.log") #Local em C:\windows\logs\software_versao.log
    <#
...
<#
    URL dos Instaladores
    Desktops/Notebooks terminados em 0-4 = SaaS01
    Desktops/Notebooks terminados em 5-9 = SaaS02
#>
    #SaaS01 32bits
    $saas01_32b = 'https://XXXXXX.manage.trendmicro.com:443/officescan/download/agent_cloud_x86.msi'

    #SaaS01 64bits
    $saas01_64b = 'https://XXXXXX.manage.trendmicro.com:443/officescan/download/agent_cloud_x64.msi'

    #SaaS02 32bits
    $saas02_64b = 'https://YYYYYY.manage.trendmicro.com:443/officescan/download/agent_cloud_x86.msi'

    #SaaS02 64bits
    $saas02_64b = 'https://YYYYYY.manage.trendmicro.com:443/officescan/download/agent_cloud_x64.msi'
...
if($Officescan -eq $true -or $installed -eq $false){
    ...
    # Verifica de endpoint eh 64 bits
    ...
    if ( ($env:computername).Substring(($env:computername).Length-1) -lt 5) {
        $name04 = $true
    }
    ...
    #Verifica se pacote esta correto
    if (-not $instalador) {
        gravarLog -logMessage "Nao foi possivel identificar instalador correto"
        #throw '[ERRO] [100] Nao foi possivel identificar o instalador correto'
        exit 100
    }        
    

    if (-not (Test-Path "$Location\osce_agent_xx.msi") ) {
        # Baixar pacote
        gravarLog -logMessage "Baixar instalador - $instalador"
        Invoke-WebRequest -Uri $instalador -OutFile "$Location\osce_agent_xx.msi"

        if (-not (Test-Path "$Location\osce_agent_xx.msi") ) {
            gravarLog -logMessage "Nao foi possivel baixar instalador $instalador"
            exit 300
        }
    }
    gravarLog -logMessage "Pacote de instalacao ja esta no local"
